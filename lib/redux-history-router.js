...
const createHistoryRouter = (routes, config = {}) => location => dispatch => {
    ...
    try {
        const allMatchingRoutes = routes.map(route => Object.assign({}, route, {
        ...
        const applicableRoutes = config.allowMultipleRoutes ? allMatchingRoutes.map(i => i) : [allMatchingRoutes.shift()];
